...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(self.body) - 1):
            ...
            right_size = (0, right_size - x.shape[-1], 0, right_size - x.shape[-2])
            feat = torch.nn.functional.pad(x, right_size, mode='constant', value=0)
...
if __name__ == '__main__':
    model = ResNetV2([3, 4, 9], 1)
    x = torch.randn(2, 3, 224, 224)
    x = model(x)

    """
    torch.Size([2, 64, 256, 256])
    feat: torch.Size([2, 256, 128, 128])
    feat: torch.Size([2, 512, 64, 64])
    
    feat: torch.Size([2, 1024, 32, 32])
    """